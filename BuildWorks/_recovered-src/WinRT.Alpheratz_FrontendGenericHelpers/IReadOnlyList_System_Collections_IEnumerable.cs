...
internal static class IReadOnlyList_System_Collections_IEnumerable
{
	...
	private unsafe static int Do_Abi_GetAt_0(nint thisPtr, uint index, nint* __return_value__)
	{
		...
		*__return_value__ = 0;
		try
		{
			enumerable = IReadOnlyListMethods<IEnumerable>.Abi_GetAt_0(thisPtr, index);
			*__return_value__ = MarshalInterface<IEnumerable>.FromManaged(enumerable);
		}
		catch (Exception ex)
		{
	...
	private unsafe static int Do_Abi_IndexOf_2(nint thisPtr, nint value, uint* index, byte* __return_value__)
	{
		...
		*__return_value__ = 0;
		...
		try
		{
			flag = IReadOnlyListMethods<IEnumerable>.Abi_IndexOf_2(thisPtr, MarshalInterface<IEnumerable>.FromAbi(value), out index2);
			*index = index2;
	...
	private unsafe static int Do_Abi_GetMany_3(nint thisPtr, uint startIndex, int __itemsSize, nint items, uint* __return_value__)
	{
		uint num = 0u;
		*__return_value__ = 0u;
		IEnumerable[] items2 = MarshalInterface<IEnumerable>.FromAbiArray((__itemsSize, items));
		try
		{
			num = IReadOnlyListMethods<IEnumerable>.Abi_GetMany_3(thisPtr, startIndex, ref items2);
			MarshalInterface<IEnumerable>.CopyManagedArray(items2, items);
	...
	private unsafe static int Do_Abi_get_Size_1(nint thisPtr, uint* __return_value__)
	{
		...
		*__return_value__ = 0u;
		try
		{
			num = IReadOnlyListMethods<IEnumerable>.Abi_get_Size_1(thisPtr);
			*__return_value__ = num;
		}
		catch (Exception ex)
		{
